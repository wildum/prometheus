...
func TestNoEmptyBlocks(t *testing.T) {
	...
	t.Run("Test no blocks after deleting all samples from head.", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NoError(t, app.Commit())
		require.NoError(t, db.Delete(context.Background(), math.MinInt64, math.MaxInt64, defaultMatcher))
		require.NoError(t, db.Compact())
	...
	compaction should not run into infinite loop (was seen during development).`, func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NoError(t, app.Commit())
		require.NoError(t, db.head.Delete(context.Background(), math.MinInt64, math.MaxInt64, defaultMatcher))
		require.NoError(t, db.Compact())
...
func TestCorrectNumTombstones(t *testing.T) {
	...
	require.NoError(t, db.Delete(context.Background(), 0, 1, defaultMatcher))
	require.Equal(t, uint64(1), db.blocks[0].meta.Stats.NumTombstones)

	// {0, 1} and {2, 3} are merged to form 1 tombstone.
	require.NoError(t, db.Delete(context.Background(), 2, 3, defaultMatcher))
	require.Equal(t, uint64(1), db.blocks[0].meta.Stats.NumTombstones)

	require.NoError(t, db.Delete(context.Background(), 5, 6, defaultMatcher))
	require.Equal(t, uint64(2), db.blocks[0].meta.Stats.NumTombstones)

	require.NoError(t, db.Delete(context.Background(), 9, 11, defaultMatcher))
